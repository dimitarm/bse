...
import cProfile, pstats, qstklearn.gendata
import numpy as np
...
class myknn(object):
    ...
    def query(self, points, k=None, method=None):
        ...
        # n_clsses = map(lambda rslt: self.data[rslt][-1], na_neighbors)
        # print n_clsses
        result = list()
        for neib in na_neighbors:
            n_clsses = map(lambda rslt: self.data[rslt][-1], neib)
            if method == 'mode':
                # return map(lambda x: scipy.stats.stats.mode(x)[0],n_clsses)[0]
                pass
            elif method == 'mean':
    #            return numpy.array(map(lambda x: numpy.mean(x),n_clsses))
                result.append(numpy.mean(n_clsses))
            elif method == 'median':
                result.append(numpy.median(n_clsses))
            elif method == 'raw':
                result.append(numpy.array(n_clsses))
            elif method == 'majority':
                class_values = numpy.unique(n_clsses)
                hist, bin_edges = numpy.histogram(n_clsses, class_values.shape[0])
                max_indice = numpy.argmax(hist)
                result.append(class_values[max_indice])
            elif method == 'all':
                result.append(numpy.array(n_clsses))            

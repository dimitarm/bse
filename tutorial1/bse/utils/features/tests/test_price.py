...
class Test(unittest.TestCase):
    ...
    def testBollinger(self):
        ...
        #taBollUp = taBollUp - 2 * taStd * ( 1 / np.sqrt(19/20) - 1)
        dfStd = pand.rolling_std(dFullData['close'], 20) * np.sqrt(19/20)
        
        #dfMean = dfResultUp - 2 * dfStd 
        
        plt.clf()
        plt.plot(dfStd.values.ravel() / taStd, color='b')
        plt.show()

...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        dData['close'] = pand.DataFrame(data=np.random.randn(10, 2) + 10, columns=('aaaaaa', 'bbb'), index=dateList)
        dData['high'] = pand.DataFrame(data=np.random.randn(10, 2) + 10, columns=('aaaaaa', 'bbb'), index=dateList)
        dData['low'] = pand.DataFrame(data=np.random.randn(10, 2) + 10, columns=('aaaaaa', 'bbb'), index=dateList)
        dData['volume'] = pand.DataFrame(data=np.random.randn(10, 2) * 5 + 100, columns=('aaaaaa', 'bbb'), index=dateList)
        pass
    ...
    def testfeatNVITradeRule(self):
        ...
        dfRule = vol.featNVITradeRule(dData)
        
    def testfeatPVITradeRule(self):
        dData = {}
        dData['close'] = pand.DataFrame(data=np.random.randn(100) + 10, columns=['aaaaaa'])
        dData['volume'] = pand.DataFrame(data=np.random.randn(100)*5 + 100, columns=['aaaaaa'])
        dfRule = vol.featPVITradeRule(dData)
        
    def testfeatPVI2SMA(self):
        dData = {}
        dData['close'] = pand.DataFrame(data=np.random.randn(100) + 10, columns=['aaaaaa'])
        dData['volume'] = pand.DataFrame(data=np.random.randn(100)*5 + 100, columns=['aaaaaa'])
        vol.featPVI2SMA(dData)
    ...
    def testNVI(self):
        dData = {}
        dData['close'] = pand.DataFrame(data=[1355.69, 1325.51, 1335.02, 1313.72, 1319.99, 1331.85, 1329.04, 1362.16, 1365.51, 1374.02, 1367.58, 1354.68, 1352.46, 1341.47, 1341.45, 1334.76, 1356.78, 1353.64, 1363.67, 1372.78, 1376.51, 1362.66, 1350.52, 1338.31, 1337.89, 1360.02, 1385.97, 1385.3, 1379.32, 1375.32], columns=['aaaaaa'])
        dData['volume'] = pand.DataFrame(data=[2739.554304, 3119.458048, 3466.876416, 2577.124096, 2480.44672, 2329.789184, 2793.069568, 3378.778368, 2417.586944, 1442.810752, 2122.5632, 2083.558016, 2103.846784, 2526.193152, 2491.886848, 2730.317312, 2311.589632, 2135.4208, 2642.721536, 2701.346816, 3122.259968, 3125.065984, 2728.037888, 2844.188672, 2810.242048, 3256.421888, 3261.50272, 2296.256, 2701.690368, 2935.853312], columns=['aaaaaa'])
        ...
        dfResult = vol.featNVI(dData, iInitValue=1000)
